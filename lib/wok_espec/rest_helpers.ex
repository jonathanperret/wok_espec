defmodule WokEspec.RESTHelpers do
  ...
  def stub_params(request, params) do
    :meck.expect(:cowboy_req, :binding, fn(param, http_show_req) ->
  ...
  defp stub_headers(request, headers) do
    :meck.expect :cowboy_req, :header, fn(header_key, request, "") ->
      headers
      |> Enum.find({}, fn({key, val}) -> header_key == key end)
      |> Tuple.to_list
      |> Enum.join(" ")
    end
  end
  ...
  defp build_route_url(route, params) do
    suffix = params |> Enum.map(fn({key, val}) -> ":#{Atom.to_string(key)}" end) |> Enum.join("/")
    case suffix do
      "" -> route
      suffix -> "#{route}/#{suffix}"
    end
  ...
  defp replace_symbols_by_values(route, params) do
    ...
    route = params |> Enum.reduce route_url, fn({key, value},acc) ->
